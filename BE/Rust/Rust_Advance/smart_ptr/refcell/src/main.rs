...
//类似于Rc<T>，RefCell<T>只能用于【单线程】场景。
//
//3、选择Box<T>、Rc<T>或RefCell<T>的理由：
//Rc<T> 允许相同数据有【多个只读】所有者；Box<T> 和 RefCell<T> 有【单一】所有者。
//Box<T> 允许在编译时执行【不可变或可变借用】检查；Rc<T>仅允许在编译时执行【不可变借用】检查；RefCell<T> 允许在【运行时】执行不可变或可变借用检查。
